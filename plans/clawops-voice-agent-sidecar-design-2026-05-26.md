...
- mapper가 Spring event payload에 채우는 `resultMessage`와 `aiSummary`는 전체 통화 전문이나 긴 transcript 후보가 아니라 공개용 짧은 결과 요약으로 제한한다. 예시는 `6월 1일 오후 7시 00분 4명 예약 성공`, `6월 1일 오후 8시 00분 대체 시간 제안받음`, `전화 예약 실패`다.
...
2026-05-31 현재 approved real-agent 실행 후보와 ClawOps 번호 inbound fallback endpoint까지 구현 / 시도했다. Spring preflight와 sidecar readiness는 통과했고 ClawOps 통화 기록상 allowlist 번호 outbound call completed도 확인했다. local dev에서는 prompt / runner 보강 후 AI 결과 tool 호출, Spring internal event delivery, event ledger 처리, call attempt 완료, 예약 `CONFIRMED` 상태 전이까지 확인한 적이 있다. AWS dev 재시도에서는 실제 발신 경로는 동작했지만 AI result tool 미제출이 반복되어 `AI_RESULT_TOOL_MISSING`이 발생했다. 현재 runner는 이 케이스를 retryable transient로 남기지 않고 `AI_PARSE_FAILED` / `NEEDS_CONFIRMATION`으로 정리하며, ClawOps/OpenAI transcript 이벤트에서 원 요청 날짜 / 시간 / 인원과 명확한 가능 답변이 확인될 때만 좁은 demo fallback으로 `CONFIRMED` 후보를 만든다. 다음 작업은 AWS sidecar 배포 후 allowlist 1회 재검증, process lifecycle 안정화, 실제 식당 번호 정책, provider call id 보존 정책 확정이다.
...
- [x] AWS dev result-tool-missing fallback / transcript diagnostics 보강
- [x] 문서 반영 완료
...
- 2026-05-31: AWS dev 환경에서 sidecar service를 별도 systemd process로 구성했고 readiness / preflight는 통과했다. 실제 allowlist 발신은 수락되었지만 AI result tool 미제출이 반복되어 `AI_RESULT_TOOL_MISSING`이 발생했다. runner에 transcript diagnostics, post-call result grace, semantic VAD low eagerness, `AI_RESULT_TOOL_MISSING -> AI_PARSE_FAILED`, transcript 기반 narrow confirmation fallback을 추가했다. 실제 allowlist 재검증은 남아 있다.
- 2026-05-31: 시연 통화 품질 보정을 위해 첫 발화에서 식당 / 지점명 확인과 AI 예약 도우미 고지를 함께 하도록 prompt를 강화했다. 날짜 / 시간 / 인원은 `6월 23일, 오후 8시 30분, 두 명`처럼 쉼표가 있는 한국어 발화 구문으로 전달하고, OpenAI Realtime turn detection은 semantic VAD low eagerness + `interrupt_response=false`로 조정해 직원 발화가 AI 발화를 자동 취소하지 않도록 했다. 이후 수신 전에 AI가 먼저 말하는 문제가 확인되어 OpenAI Realtime `greeting=false`로 변경했다. 추가로 직원이 먼저 말하지 않으면 AI가 시작하지 않는 문제가 확인되어, `create_response=false`와 sidecar-managed `response.create()`를 적용했다. sidecar는 SDK `call_start` event 후 1초 뒤 또는 직원 transcript 완료 시점에 응답 생성을 요청한다.
- 2026-06-03: AWS dev allowlist 1회 재검증에서 `call_start` 후 1초 응답 요청, AI 도우미 고지, 지점 확인, 예약 가능 질문, 명확한 가능 답변, Spring `CONFIRMED` / `AI_CONFIRMED`, provider event `PROCESSED`, call attempt `COMPLETED`를 확인했다. 지점 확인 후 불필요한 filler 없이 바로 availability 질문만 하도록 prompt를 추가로 좁혔고, 상대가 “여보세요?”만 답한 경우 전체 AI 소개를 반복하지 않고 지점 확인만 다시 묻도록 per-response 지시를 추가했다. live transcript fallback 또는 result tool 수락 뒤에는 closing response를 명시적으로 요청하고 closing grace를 늘려 마무리 멘트 전에 끊기지 않도록 보강했다. 서버에서 readiness / preflight / AI-call reservation create / polling을 반복 실행할 수 있는 `scripts/run_dev_ai_call_smoke.py` helper도 추가했다.
- 2026-06-03: 최신 ClawOps transcript 점검에서 지점 확인 뒤 대기는 개선됐지만, availability 질문 이후 직원의 명확한 가능 응답 transcript 없이 closing으로 넘어간 케이스를 확인했다. `CONFIRMED` result tool 수락과 transcript fallback 모두 availability 질문 이후 고객 확답이 있어야만 확정 후보를 만들도록 보강했다. 지점 확인 응답만으로는 확정 처리하지 않는다.
- 2026-06-03: 후속 live transcript에서 직원이 “예약자 이름을 알려달라”고 요청했는데 AI가 이름을 제공하지 않고 closing으로 넘어가는 문제가 확인되었다. prompt와 SDK runner transcript gate를 보강해 식당이 예약자명 / 연락처를 요구하면 시스템 제공 값을 먼저 전달하고 직원 확인을 받은 뒤에만 `CONFIRMED` 후보를 만들도록 했다.
- 2026-06-03: 불가 답변을 바로 `UNAVAILABLE`로 닫지 않고 가능한 대체 시간대가 있는지 한 번 묻도록 sidecar response gate와 transcript fallback을 보강했다. 대체 시간이 제안되면 `NEEDS_CONFIRMATION`, 없다고 확인되면 `UNAVAILABLE` 후보로 처리한다.
- 2026-06-03: 직원의 한 문장이 transcript 여러 조각으로 들어와 `네 가능합니다` 이후 `예약자 이름 말해주세요`가 늦게 도착하면 AI가 이름 제공 없이 closing으로 넘어갈 수 있는 문제가 확인되었다. `CONFIRMED` result tool / live transcript fallback에 최신 직원 transcript 안정화 대기를 추가했고, split name-request 응답 테스트를 보강했다.
...
- 실제 allowlist 테스트 번호 발신 / 통화 완료는 확인했다. local dev에서는 AI 결과 tool 제출과 Spring 예약 상태 전이까지 확인한 적이 있으나, AWS dev 최신 경로는 transcript fallback 보강 후 재검증이 남아 있다. 남은 범위는 AWS sidecar 재검증, 운영 배포 안정화, 실제 식당 번호 사용 정책 확정이다.
