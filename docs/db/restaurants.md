...
`tags`는 식당 검색/상세 보조 신호로 쓰는 태그 마스터이고, `restaurant_tags`는 식당과 태그의 연결 정보다.

태그는 아래 prefix를 사용한다.

- `category:*`: 한식, 카페/디저트, 냉면 전문처럼 카테고리와 업종에서 파생한 태그
- `menu:*`: 실제 메뉴명에서 파생한 태그
- `feature:*`: 주차 가능, 포장 가능, 단체 가능처럼 편의정보에서 파생한 태그
- `style:*`: 혼밥, 데이트, 술자리처럼 이용 맥락에서 파생한 태그
- `taste:*`: 매운맛, 든든한 한끼, 면요리처럼 메뉴/업종 조합에서 파생한 태그
