...
- mapper의 `resultMessage`와 `aiSummary`는 전체 통화 내용이 아니라 공개용 짧은 결과 요약으로 제한했다. 실제 raw transcript 저장 정책은 여전히 미확정이며 현재 범위 밖이다.
...
- dev real-agent readiness checker는 `sidecars/clawops-voice-agent/scripts/check_dev_real_agent_readiness.py`에 추가했다. 공개 dev URL, 선택적 Spring preflight, 선택적 sidecar readiness를 확인하지만 실제 ClawOps/OpenAI 호출, Voice Agent 실행, 전화 발신은 하지 않고 secret / 원본 번호도 출력하지 않는다.
...
- 2026-05-27~28 local dev PoC에서 Spring `dev,db,key` profile, `ddl-auto=validate`, Python 3.12 sidecar real-agent 설정으로 승인된 allowlist 발신을 수행했다. Spring preflight와 sidecar readiness는 통과했고 ClawOps 통화 기록상 outbound call completed와 provider call id 생성까지 확인했다. 초반에는 자동 안내 / 불명확 응답을 AI가 너무 빨리 `FAILED`로 판단해 `PROVIDER_TRANSIENT_ERROR` event와 `RETRY_SCHEDULED` attempt가 기록되었다.
- Voice Agent 결과 tool 호출 안정화를 위해 prompt에 `submit_reservation_call_result` 필수 제출 규칙을 명시하고, SDK runner는 결과 tool 제출과 call end를 race로 기다리도록 보강했다. 이후 `partySize` 문자열 coercion, boolean field coercion, sidecar id 기반 Spring event matching, `send_dtmf` 허용, 직원 질문 응답 규칙을 추가했다. 추가로 연결 확인성 요약만 있는 result, 원 예약 날짜 / 시간 / 인원 수를 요약하지 않은 terminal result, 원 요청과 충돌하는 `CONFIRMED` result는 tool 단계에서 거절하도록 보강했다. 2026-05-29 확인된 추가 문제에 따라 availability 질문 직후 곧바로 “들리시나요”로 넘어가지 않도록 prompt를 강화했고, 식당이 명확히 가능하다고 답한 경우 45초까지 억지로 재확인하지 않으며, accepted result 뒤에는 짧은 closing grace 동안 방문 확인 멘트를 할 수 있게 했다. 2026-05-30~31 AWS dev 재시도에서는 실제 발신 경로는 동작했지만 AI result tool 미제출이 반복되어 `AI_RESULT_TOOL_MISSING`이 발생했다. 이를 retryable transient가 아니라 `AI_PARSE_FAILED` / `NEEDS_CONFIRMATION`으로 정리하고, ClawOps/OpenAI transcript 이벤트에 원 요청 날짜 / 시간 / 인원과 명확한 가능 답변이 모두 있는 경우에만 좁은 demo fallback으로 `CONFIRMED` 후보를 만들도록 보강했다. 2026-05-31에는 첫 발화에서 식당 / 지점명 확인과 AI 도우미 고지를 함께 하도록 prompt를 강화하고, 날짜 / 시간 / 인원은 쉼표가 있는 한국어 발화 구문으로 천천히 말하도록 했으며, `interrupt_response=false`로 AI 발화 중 직원 말이 들어와도 자동 취소되지 않도록 조정했다. 추가로 `greeting=false`로 변경해 Realtime session 시작 직후 수신 전 선발화가 발생하지 않도록 했다. 이후 직원이 먼저 말하지 않으면 AI가 시작하지 않는 문제가 확인되어, `create_response=false`로 자동 VAD 응답 생성을 끄고 sidecar가 SDK `call_start` event 후 1초 뒤 또는 직원 transcript 완료 시점에 명시적으로 `response.create()`를 요청하도록 보강했다. 2026-06-03 AWS dev allowlist 1회 재검증에서 `call_start` 후 1초 응답 요청, 지점 확인, 예약 가능 질문, Spring `CONFIRMED` / `AI_CONFIRMED`, event ledger `PROCESSED`, call attempt `COMPLETED`를 확인했다. 이후 지점 확인 뒤에는 filler 없이 바로 availability 질문만 하도록 prompt를 더 좁혔고, 상대가 “여보세요?”만 답한 경우 전체 AI 소개를 반복하지 않고 지점 확인만 다시 묻도록 보정했다. live transcript fallback 또는 result tool 수락 뒤에는 closing response를 명시적으로 요청하고 closing grace를 늘려 마무리 멘트 전에 끊기지 않도록 보강했다. 서버 데모 반복 실행용 `run_dev_ai_call_smoke.py` helper도 추가했다. 최신 transcript 점검에서 지점 확인 대기는 개선됐지만 availability 질문 이후 고객의 명확한 가능 답변 없이 closing으로 넘어간 케이스가 확인되어, `CONFIRMED` result tool과 transcript fallback 모두 availability 질문 이후 고객 확답이 있어야만 확정 후보를 만들도록 보강했다. 이어서 식당이 “예약자 이름 / 연락처를 알려달라”고 요청한 경우 AI가 시스템 제공 예약자명 / 연락처를 먼저 전달하고 직원 확인을 받은 뒤에만 `CONFIRMED` 후보를 만들도록 추가 보강했다. 2026-06-03 후속 점검에서 직원 발화가 `네 가능합니다`와 `예약자 이름 말해주세요`로 쪼개져 들어오는 경우가 확인되어, 최신 직원 transcript 직후 `CONFIRMED` 후보를 짧게 안정화 대기한 뒤 수락하도록 보강했다. 또한 요청 시간이 불가하다는 답변을 받으면 바로 `UNAVAILABLE`로 닫지 않고 가능한 대체 시간대가 있는지 한 번 묻고, 대체 시간이 있으면 `NEEDS_CONFIRMATION`, 없다고 확인되면 `UNAVAILABLE` 후보로 처리하도록 보강했다.
- 남은 sidecar gap은 Python process lifecycle / 배포 방식, 실제 sidecar event delivery / retry / ack 운영 구현, 실제 ClawOps provider call id를 sidecar id와 별도로 보존할 DB / DTO 정책, 운영 관측성 / 알림, 실제 식당 번호 사용 승인 정책이다.
...
- 실제 ClawOps Voice Agent 연동은 AWS dev allowlist 기준으로 Agent SDK mode 실발신 / Spring event ledger 연동까지 확인했다. 남은 gap은 실제 식당 번호 사용 승인 정책, 운영 배포 / process lifecycle, 운영 관측성, 실패 알림, sidecar event delivery / retry / ack 운영 구현이다.
