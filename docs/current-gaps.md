...
- 남은 sidecar gap은 ClawOps control websocket / Voice Agent 결과 tool 호출 안정화, 실제 OpenAI Realtime 통화 결과 확인, Python process lifecycle / 배포 방식, 실제 sidecar event delivery / retry / ack 운영 구현, 실제 provider call id 수신 후 상태 전이 기준이다.
