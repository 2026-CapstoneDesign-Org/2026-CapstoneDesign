...
- Voice Agent 결과 tool 호출 안정화를 위해 prompt에 `submit_reservation_call_result` 필수 제출 규칙을 명시하고, SDK runner는 결과 tool 제출과 call end를 race로 기다리도록 보강했다. 이후 `partySize` 문자열 coercion, boolean field coercion, sidecar id 기반 Spring event matching, `send_dtmf` 허용, 직원 질문 응답 규칙을 추가했다. 추가로 연결 확인성 요약만 있는 result, 원 예약 날짜 / 시간 / 인원 수를 요약하지 않은 terminal result, 원 요청과 충돌하는 `CONFIRMED` result는 tool 단계에서 거절하도록 보강했다. 2026-05-29 확인된 추가 문제에 따라 availability 질문 직후 곧바로 “들리시나요”로 넘어가지 않도록 prompt를 강화했고, 식당이 명확히 가능하다고 답한 경우 45초까지 억지로 재확인하지 않으며, accepted result 뒤에는 짧은 closing grace 동안 방문 확인 멘트를 할 수 있게 했다. 최신 local dev allowlist 테스트에서는 AI가 연결 확인 직후 바로 끊지 않고 직원의 가능 답변을 받은 뒤 `RESERVATION_CONFIRMED` event를 만들었고, Spring event ledger `PROCESSED`, 예약 `CONFIRMED`, call attempt `COMPLETED`까지 확인했다.
