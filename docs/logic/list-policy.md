...
- 리스트에서 식당 제거
- 리스트 안 식당 점수 수정
- 리스트 상세 / 대표 리스트 조회 시 식당 목록 `autoScore` 내림차순 정렬
...
### 3-7. 리스트 조회 정렬
- `GET /lists/{id}`와 `GET /lists/users/{id}/representative` 응답의 식당 목록은 `autoScore`가 높은 순서로 반환한다.
- API 경로와 응답 필드는 바꾸지 않고 내부 응답 매핑에서만 정렬한다.
